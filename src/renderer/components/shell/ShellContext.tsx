...
interface ShellContextValue {
  ...
  registerWindow(
    key: ShellWindowKey,
    metadata?: Partial<ShellWindowMetadata>
  ): void;
...
const BASE_WINDOW_METADATA: Record<ShellWindowKey, ShellWindowMetadata> =
  Object.keys(SHELL_WINDOWS).reduce((acc, rawKey) => {
...
export const ShellProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [windowStates, setWindowStates] = useState<
    Partial<Record<ShellWindowKey, unknown>>
  >({});
  const [minimizedWindows, setMinimizedWindows] = useState<ShellWindowKey[]>(
    []
  );
  const [windowMetadata, setWindowMetadataState] =
    useState<Record<ShellWindowKey, ShellWindowMetadata>>(BASE_WINDOW_METADATA);
