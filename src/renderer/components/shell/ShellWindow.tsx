...
export const ShellWindow: React.FC<ShellWindowProps> = ({
  ...
  const containerBaseClass = hideChrome
    ? 'relative mt-1.5 flex min-h-0 flex-col transition-all duration-200'
  ...
  const containerClassName = cn(
    containerBaseClass,
    chromeClassName,
    hideChrome ? 'w-full h-[78vh]' : sizeClass
  );

  const contentChromeClass = hideChrome
    ? 'flex-1 min-h-0 overflow-hidden transition-opacity duration-200'
    : 'flex-1 min-h-0 overflow-hidden border border-hol-border bg-hol-surface-alt transition-opacity duration-200';
