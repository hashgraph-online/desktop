...
import AnimatedSuggestions from '../chat/headers/AnimatedSuggestions';
...
type AssistantDockPlacement = 'left' | 'right' | 'bottom';
...
const QuickActionCard: React.FC<QuickActionCardProps> = (props) => {
  const { action, onSelect } = props;
  const handleClick = useCallback(() => {
    onSelect(action.value);
  }, [action.value, onSelect]);

  return (
    <button
      type='button'
      onClick={handleClick}
      className='group flex h-24 flex-col justify-between rounded-2xl border border-gray-200/60 dark:border-gray-700/60 bg-white/80 dark:bg-gray-900/80 px-4 py-3 text-left transition-all duration-200 hover:border-purple-300/70 dark:hover:border-purple-600/70 hover:shadow-[0_20px_48px_rgba(139,92,246,0.2)] hover:bg-purple-50/80 dark:hover:bg-purple-950/40 backdrop-blur-sm'
    >
      <Typography
        variant='caption'
        className='text-[11px] uppercase tracking-[0.24em] text-gray-600 dark:text-gray-400 font-medium'
      >
        {action.label}
      </Typography>
      <Typography
        variant='body2'
        className='text-sm text-gray-700 dark:text-gray-300 leading-snug group-hover:text-gray-900 dark:group-hover:text-white transition-colors duration-200'
      >
        {action.description}
      </Typography>
    </button>
...
const BrowserAssistantPanel: React.FC<BrowserAssistantPanelProps> = (props) => {
  ...
  const chatContext = useAgentStore((state) => state.chatContext);
  ...
  const buildBrowserAttachments = useCallback(
    async ({ addNotification }: ExtraAttachmentsBuilderArgs) => {
      ...
      try {
        ...
        const truncatedExcerpt = context?.text
          ? truncateContext(context.text, 2000)
          : undefined;
        const contextPayload: Record<string, unknown> = {
          url: currentUrl,
        };
        if (pageTitle.trim()) {
          contextPayload.title = pageTitle.trim();
        }
        if (context?.description?.trim()) {
          contextPayload.description = context.description.trim();
        }
        if (context?.selection?.trim()) {
          contextPayload.selection = truncateContext(context.selection, 1500);
        }
        if (truncatedExcerpt) {
          contextPayload.excerpt = truncatedExcerpt;
        }
        if (hostLabel) {
          contextPayload.host = hostLabel;
        }
    ...
    [fetchPageContext, currentUrl, pageTitle, hostLabel]
  );
  ...
  const handleSessionActivated = useCallback(
    ...
    [onSessionCreated, sessionId, setChatContext, hostLabel, pageTitle]
  );
  ...
  const {
    ...
    buildSessionName: () => buildSessionName(hostLabel, pageTitle),
  ...
  useEffect(() => {
    ...
    const assistantLabel = hostLabel || pageTitle || 'Moonscape Assistant';
    setChatContext({
      mode: 'personal',
      agentName: assistantLabel,
    });
  }, [isOpen, hostLabel, pageTitle, setChatContext]);
  ...
  useEffect(() => {
    ...
    const assistantLabel = currentSession.name || hostLabel || pageTitle;
    setChatContext({
      mode: 'personal',
      agentName: assistantLabel || 'Moonscape Assistant',
    });
  ...
  }, [isOpen, sessionId, currentSession?.id, loadSession, addNotification]);
  const activeMessages = useMemo(() => {
    ...
    return messages;
  }, [messages, chatContext.mode, currentSession?.id, sessionId]);
