...
export default function DeleteSessionDialog(props: DeleteSessionDialogProps) {
  const { sessionId, onCancel, onConfirm } = props;
  if (!sessionId) {
    return null;
  }
  return (
        ...
        <div className='flex items-start gap-4'>
          ...
          <div className='flex-1'>
            ...
            <Typography variant='body2' color='muted' className='mb-6'>
              Are you sure you want to delete this session? This action cannot be undone and all messages will be permanently deleted.
            </Typography>
            <div className='flex gap-3 justify-end'>
              <Button variant='ghost' onClick={onCancel} className='px-4 py-2'>
                Cancel
              </Button>
              <Button variant='destructive' onClick={() => onConfirm(sessionId)} className='px-4 py-2 bg-red-600 hover:bg-red-700 text-white'>
                Delete Session
              </Button>
            </div>
          </div>
