...
import { OperationalMode } from '@hashgraphonline/conversational-agent';
...
export class ConfigService {
  ...
  async load(): Promise<AppConfig> {
    try {
      ...
      return this.deepMergeWithDefaults(configData, true);
    } catch (error) {
  ...
  private async performAtomicSave(config: AppConfig): Promise<void> {
    ...
    try {
      await this.logTransaction(transactionId, 'start', {
        tempPath,
        backupPath,
      });
      ...
      if (safeStorage.isEncryptionAvailable()) {
        if (
          configToSave.hedera?.privateKey &&
          configToSave.hedera.privateKey.trim() !== ''
        ) {
        ...
        if (
          configToSave.openai?.apiKey &&
          configToSave.openai.apiKey.trim() !== ''
        ) {
        ...
        if (
          configToSave.anthropic?.apiKey &&
          configToSave.anthropic.apiKey.trim() !== ''
        ) {
        ...
        this.logger.warn(
          'Encryption not available, saving config in plain text'
        );
      }
      ...
      await this.logTransaction(transactionId, 'write_temp', {
      ...
      try {
        ...
        if (
          !parsedData.hedera ||
          !parsedData.openai ||
          !parsedData.anthropic ||
          !parsedData.advanced
        ) {
          throw new Error('Invalid config structure after write');
        }
      } catch (error) {
        await this.logTransaction(transactionId, 'validation_failed', {
          error: String(error),
        });
      ...
      this.logger.info('Config saved successfully with atomic write', {
        transactionId,
      });
    } catch (error) {
      this.logger.error('Failed to save config, attempting rollback', {
        transactionId,
        error,
      });
  ...
  private async rollbackTransaction(
    ...
    try {
      ...
      await this.logTransaction(transactionId, 'rollback_failed', {
  ...
  private async recoverFromTransactionLog(): Promise<void> {
    try {
      ...
      this.logger.warn('Found incomplete transaction, attempting recovery', {
        transactionId,
        state,
      });
      ...
      switch (state) {
        ...
        case 'backup_created':
          if (data?.backupPath && fs.existsSync(data.backupPath)) {
            ...
            this.logger.info(
              'Restored config from backup after incomplete transaction'
            );
          }
          if (data?.tempPath && fs.existsSync(data.tempPath)) {
            await fs.promises.unlink(data.tempPath);
        ...
        case 'renamed':
          if (data?.backupPath && fs.existsSync(data.backupPath)) {
            ...
            this.logger.info(
              'Cleaned up backup file from successful transaction'
            );
          }
          break;
        ...
        case 'rollback_failed':
          this.logger.error(
            'Transaction rollback was incomplete, manual recovery may be needed',
            { transactionId }
          );
          break;
      }
  ...
  private getDefaultConfig(): AppConfig {
    ...
    if (process.env.HEDERA_OPERATOR_ID) {
      envVarsFound.push('HEDERA_OPERATOR_ID');
    }
    if (process.env.HEDERA_OPERATOR_KEY) {
      envVarsFound.push('HEDERA_OPERATOR_KEY');
    }
    if (process.env.HEDERA_NETWORK) {
      envVarsFound.push('HEDERA_NETWORK');
    }
    if (process.env.OPENAI_API_KEY) {
      envVarsFound.push('OPENAI_API_KEY');
    }
    if (process.env.ANTHROPIC_API_KEY) {
      envVarsFound.push('ANTHROPIC_API_KEY');
    }

    if (envVarsFound.length > 0) {
      this.logger.info('Using environment variables for configuration', {
        variables: envVarsFound,
      });
    }
  ...
  private deepMergeWithDefaults(
    ...
    if (includeEnvVars) {
      if (process.env.HEDERA_OPERATOR_ID && !merged.hedera.accountId) {
        merged.hedera.accountId = process.env.HEDERA_OPERATOR_ID;
      }
      if (process.env.HEDERA_OPERATOR_KEY && !merged.hedera.privateKey) {
        merged.hedera.privateKey = process.env.HEDERA_OPERATOR_KEY;
      }
      if (process.env.HEDERA_NETWORK && !merged.hedera.network) {
        merged.hedera.network = process.env.HEDERA_NETWORK as
          | 'mainnet'
          | 'testnet';
      }
      if (process.env.OPENAI_API_KEY && !merged.openai.apiKey) {
      ...
      if (process.env.LLM_PROVIDER && !merged.llmProvider) {
        const provider = process.env.LLM_PROVIDER.toLowerCase();
        if (provider === 'openai' || provider === 'anthropic') {
          merged.llmProvider = provider as 'openai' | 'anthropic';
        }
      }
    }

    if (merged.hedera.accountId === undefined)
      merged.hedera.accountId = defaults.hedera.accountId;
    if (merged.hedera.privateKey === undefined)
      merged.hedera.privateKey = defaults.hedera.privateKey;
    if (merged.hedera.network === undefined)
      merged.hedera.network = defaults.hedera.network;

    if (merged.openai.apiKey === undefined)
      merged.openai.apiKey = defaults.openai.apiKey;
    if (merged.openai.model === undefined)
      merged.openai.model = defaults.openai.model;

    if (merged.anthropic.apiKey === undefined)
      merged.anthropic.apiKey = defaults.anthropic.apiKey;
    if (merged.anthropic.model === undefined)
      merged.anthropic.model = defaults.anthropic.model;

    if (merged.advanced.theme === undefined)
      merged.advanced.theme = defaults.advanced.theme;
    if (merged.advanced.autoStart === undefined)
      merged.advanced.autoStart = defaults.advanced.autoStart;
    if (merged.advanced.logLevel === undefined)
      merged.advanced.logLevel = defaults.advanced.logLevel;
